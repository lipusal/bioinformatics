...
# Compute the convolution of a peptide's spectrum (molecular masses read from all chunks of a peptide).
# Corresponds to lesson 4.9
def convolution(spectrum, restrict = false)
  ...
  multiplicities
      ...
      .select { |diff, _multiplicity| !restrict || diff >= 57 && diff <= 200 } # Restrict masses if requested
...
# Like convolution, but rather than returning multiplicities prints out the convolution matrix for easier
# visualization/debugging. The format of the matrix matches that of the book. Corresponds to lesson 4.9
...
result = convolution(spectrum).join(' ')
puts result
